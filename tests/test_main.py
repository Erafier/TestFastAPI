...
def test_insurance_calculation_incorrect_date():
    response = client_test.get('/insurance?declared_value=100000&cargo_type=Glass&date=2021-10-10')
    assert response.status_code == 200
    assert response.json() == "Для даты 2021-10-10 не указан тариф на товар Glass"


def test_insurance_calculation_incorrect_values():
    response = client_test.get('/insurance?declared_value=sss&cargo_type=132&date=11111-10-10')
    assert response.status_code == 422
...
def test_insurance_calculation_correct_date():
    ...
    response = client_test.get(f'/insurance?declared_value={declared_value}&cargo_type={cargo_type}&date={date}')
